...
class FlaskTestCase(unittest.TestCase):
    ...
    def test_A_index(self):
        ...
        response = requests.get(ride_url+'/cc', headers={'Content-Type': 'html/text'})
        self.assertTrue(b'Welcome to our submission for Assignment 1' in response.content)
    ...
    def test_B1_insert_user(self):
        ...
        response = requests.put(user_url+'/api/v1/users', data=json.dumps(info), headers={'Content-Type': 'application/json'})
    ...
    def test_C1_insert_ride(self):
        ...
        response = requests.post(ride_url+'/api/v1/rides', data=json.dumps(info), headers={'Content-Type': 'application/json'})
        self.assertEqual(response.status_code,201)
    ...
    def test_D1_list_rides(self):
        tester = app.test_client(self)
        response = requests.get(ride_url+'/api/v1/rides?source={source}&destination={destination}'.format(source=3,destination=197))
        self.assertEqual(response.status_code,200)

    # Test for valid but incorrect source and destination
    def test_D2_list_rides(self):
        tester = app.test_client(self)
        response = requests.get(ride_url+'/api/v1/rides?source={source}&destination={destination}'.format(source=1,destination=2))
        self.assertEqual(response.status_code,204)

    # Test for invalid source upper bound
    def test_D3_list_rides(self):
        tester = app.test_client(self)
        response = requests.get(ride_url+'/api/v1/rides?source={source}&destination={destination}'.format(source=200,destination=197))
        self.assertEqual(response.status_code,400)

    # Test for invalid destination upper bound
    def test_D4_list_rides(self):
        tester = app.test_client(self)
        response = requests.get(ride_url+'/api/v1/rides?source={source}&destination={destination}'.format(source=3,destination=200))
        self.assertEqual(response.status_code,400)

    # Test for invalid source lower bound
    def test_D5_list_rides(self):
        tester = app.test_client(self)
        response = requests.get(ride_url+'/api/v1/rides?source={source}&destination={destination}'.format(source=0,destination=197))
        self.assertEqual(response.status_code,400)

    # Test for invalid destination lower bound
    def test_D6_list_rides(self):
        tester = app.test_client(self)
        response = requests.get(ride_url+'/api/v1/rides?source={source}&destination={destination}'.format(source=3,destination=0))
        self.assertEqual(response.status_code,400)

    # Test correct given ride list
    def test_E1_given_ride(self):
        tester = app.test_client(self)
        response = requests.get(ride_url+'/api/v1/rides/{rideId}'.format(rideId=1))
        self.assertEqual(response.status_code,200)

    # Test for nonexisting rideId
    def test_E2_given_ride(self):
        tester = app.test_client(self)
        response = requests.get(ride_url+'/api/v1/rides/{rideId}'.format(rideId=999))
        self.assertEqual(response.status_code,204)
    ...
    def test_F1_join_ride(self):
        tester = app.test_client(self)
        info = {"username":"UTest"}
        response = requests.post(ride_url+'/api/v1/rides/{rideId}'.format(rideId=1),data=json.dumps(info),headers={'Content-Type': 'application/json'})
        self.assertEqual(response.status_code,200)
    ...
    def test_F2_join_ride(self):
        ...
        response = requests.post(ride_url+'/api/v1/rides/{rideId}'.format(rideId=2),data=json.dumps(info),headers={'Content-Type': 'application/json'})
        self.assertEqual(response.status_code,400)
    ...
    def test_F3_join_ride(self):
        tester = app.test_client(self)
        info = {"username":"UTest"}
        response = requests.post(ride_url+'/api/v1/rides/{rideId}'.format(rideId=999),data=json.dumps(info),headers={'Content-Type': 'application/json'})
        self.assertEqual(response.status_code,400)

    # Test correct delete ride
    def test_G1_delete_ride(self):
        tester = app.test_client(self)
        response = requests.delete(ride_url+'/api/v1/rides/{rideId}'.format(rideId=1))
        self.assertEqual(response.status_code,200)

    # Test for nonexisting rideId
    def test_G2_delete_ride(self):
        tester = app.test_client(self)
        response = requests.delete(ride_url+'/api/v1/rides/{rideId}'.format(rideId=999))
        self.assertEqual(response.status_code,400)

    # Test for correct delete user
    def test_H1_delete_user(self):
        tester = app.test_client(self)
        response = requests.delete(user_url+'/api/v1/users/{username}'.format(username="UTest"))
        self.assertEqual(response.status_code,200)

    # Test for deleting non existing user
    def test_H2_delete_user(self):
        tester = app.test_client(self)
        response = requests.delete(user_url+'/api/v1/users/{username}'.format(username="UTest_dup"))
        self.assertEqual(response.status_code,400)
